...
export class ProductsServiceExtra {
  ...
  changeProduct(codeProduct: string): void {
    ...
    if (this.product && codeProduct === this.product.codeProduct) {
      ...
      if (this.couponInternalService.productHasCoupon && this.couponInternalService.productHasCoupon.checked &&
  ...
  destroyCouponConfirm(): void {
    this.productNameSelectedSubscribe.next(
      this.couponInternalService.productHasCoupon.productCodeRequest
    );
    this.router.getRouter().navigate(['/products/main']);
